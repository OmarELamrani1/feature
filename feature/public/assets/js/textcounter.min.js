    ...
    $.textcounter = function (el, options) {
        ...
        base.init = function () {
            ...
            var counterText = base.options.countDown
                ...
                $count_overflow_text = $("<div/>").addClass(
                    base.options.countOverflowContainerClass
                );
        ...
        base.checkLimits = function (e) {
                    ...
                    if (base.options.stopInputAtMaximum) {
                        ...
                        if (base.options.type == "word") {
                            ...
                            var maxLimit = base.options.twoCharCarriageReturn
                                ? base.options.max -
                                  base.twoCharCarriageReturnCount($text)
                                : base.options.max;
            ...
            if (
                base.options.minDisplayCutoff == -1 &&
                base.options.maxDisplayCutoff == -1
            ) {
                base.$container.show();
            } else if (
                textCount <=
                base.options.min + base.options.minDisplayCutoff
            ) {
                base.$container.show();
            } else if (
                base.options.max !== -1 &&
                textCount >= base.options.max - base.options.maxDisplayCutoff
            ) {
                base.$container.show();
            } else {
                base.$container.hide();
        ...
        (base.setOverflowMessage = function () {
            base.hideMessage(
                base.$container.find("." + base.options.textCountMessageClass)
            );
            base.removeOverflowMessage();
            var overflowText = base.options.countOverflowText
                .replace(
                    "%d",
                    base.textCount(base.$el.val()) - base.options.max
                )
                .replace("%type", base.options.type + "s");
            var overflowDiv = base.$container
                .find("." + base.options.countOverflowContainerClass)
                .append(overflowText);
            ...
            (base.clearErrors = function (type) {
                ...
                if ($countEl.children(".error-text").length == 0) {
                    ...
                    base.showMessage(
                        base.$container.find(
                            "." + base.options.textCountMessageClass
                        )
                    );
